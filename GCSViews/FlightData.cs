...
// square
        ...
        //////////////////      ITUNOM 2018 AUVSI SUAS 2018     //////////////////

        bool show_panel = true;
        bool firstLoop = true;
        bool interop_process = false;
        ////
        ...
        {
            public MovingObstacles[] moving_obstacles { get; set; }
            public StationaryObstacles[] stationary_obstacles { get; set; }
        }
        ...
        public class StationaryObstacles
        {
        ...
        public class Mission
        {
            ...
            public OffAxisTargetPos off_axis_odlc_pos { get; set; }
            public EmergentLastKnownPos emergent_last_known_pos { get; set; }
            public List<SearchGridPoint> search_grid_points { get; set; }
        }

        //////////////////Create Overlay
        GMapOverlay Overlay = new GMapOverlay("missions_polygons");
        GMapOverlay OverlayObstacles = new GMapOverlay("obstacles");

        ///////////     GMAP MARKERS    /////////// 
        //Air Drop
        GMarkerGoogle AirDropMarker;
        //Off Axis
        GMarkerGoogle OffAxisMarker;
        //Emergent Last Known Position
        GMarkerGoogle EmergentMarker;
        //Flight Zone
        GMapPolygon flightZone;
        //SearchArea
        GMapPolygon searchAreaZone;
        //WayPoints
        GMapPolygon wayPointsZone;
        //Obstacles
        ...
        string login_username = "testuser";
        string login_pass = "testpass";
        /////////////////


        private void itunom_Click(object sender, EventArgs e)///Kontrol Panelini acip kapatma butonu
        {
            if (show_panel == true)
            {
                itunom_panel.Hide();
        ...
        private void interop_start_Click(object sender, EventArgs e)
        {
            

            if (interop_process == false)
            {
                interop_process = true;
                Thread Interop_thread = new Thread(interop);
                Interop_thread.Start();
                interop_start.Text = "Stop Interoperability";
            }
            else if (interop_process == true)
            {
                ///Interop_thread.Abort();
                interop_process = false;
                interop_start.Text = "Start Interoperability";
            }
        }

        private void interop()///INTEROPERABILITY///
        {
            ///process_richtextbox.Text += "Interoperability Started\n";
            while (interop_process == true)
            {
                ...
                using (var client = new CookieWebClient())
                {
                    try
                    {
                        var response = client.UploadValues("http://" + interop_ip.Text + "/api/login", "POST", loginData);
                        var responseString = Encoding.Default.GetString(response);
                        logging_richtextbox.Text += responseString;               
                    }
                    catch
                    {
                        ///process_richtextbox.Text += "Interoperability can not log in\n";
                        interop_process = false;
                        ///interop_start.Text = "Start Interoperability";
                    }

                    ///     LOGGING AND MARKING ON MAP       ///
                    long timestart = MainV2.comPort.MAV.cs.datetime.Ticks;
                    double lasttime = 0;
                    while (interop_process == true)
                    {
                        try
                        {
                            ...
                            ///DOWNLOADING SERVER INFO
                            string responseString = client.DownloadString("http://"+interop_ip.Text+"/api/server_info");
                            logging_richtextbox.Text += responseString;
                            
                            ServerInfo svr = JsonConvert.DeserializeObject<ServerInfo>(responseString);
                            /*
                            Server_Info_message.Text = svr.message;
                            Server_Info_message_timestamp.Text = svr.message_timestamp;
                            Server_Info_time.Text = svr.server_time;
                            */


                            ///////DOWNLOAD OBSTACLES
                            string json = client.DownloadString("http://" + interop_ip.Text + "/api/obstacles");
                            all_obstacles = JsonConvert.DeserializeObject<Obstacles>(json);
                            OverlayObstacles.Markers.Clear();

                            String sobs = "";/// Saving Stationary Obstacles to a String
                            sobs = String.Format("ObstacleId\tCylinderHeight\tCylinderRadious\tLatitude\tLongitude" + Environment.NewLine);

                            int k = 1;
                            foreach (StationaryObstacles i in all_obstacles.stationary_obstacles)
                            {
                                ///Show Obstacles at Map
                                if (ObtaclesOverlay.Checked)
                                {
                                    i.marker = new GMarkerGoogle(new PointLatLng(i.latitude, i.longitude), GMarkerGoogleType.red_dot);
                                    OverlayObstacles.Markers.Add(i.marker);
                                }
                                sobs += String.Format("{0}\t{1}\t{2}\t{3}\t{4}" + Environment.NewLine, k.ToString(), i.cylinder_height.ToString(), i.cylinder_radius.ToString(), i.latitude.ToString(), i.longitude.ToString());
                                k++;
                            }

                            ////////////////BURADA STATIONARY OBSTACLES BIR TXT DOSYASINA YAZDIRILABILIR/////////////
                            ////////////////////////////////
                            ///////////////////////////////////

                            foreach (MovingObstacles i in all_obstacles.moving_obstacles)
                            {
                                ///Show Obstacles at Map
                                if (ObtaclesOverlay.Checked)
                                {
                                    i.marker = new GMarkerGoogle(new PointLatLng(i.latitude, i.longitude), GMarkerGoogleType.red_dot);
                                    //i.marker.Position = new PointLatLng(i.latitude, i.longitude);
                                    OverlayObstacles.Markers.Add(i.marker);
                                }
                            }
                            ...
                            ////        DOWNLOAD MISSION        ////
                            string missionsJSON = client.DownloadString("http://" + interop_ip.Text + "/api/missions");
                            missions = JsonConvert.DeserializeObject<List<Mission>>(missionsJSON);

                            /////
                            String misS = "";/// Saving Mission Waypoints to a String
                            misS = String.Format("WaypointOrder\tAltitudeMsl\tLatitude\tLongitude" + Environment.NewLine);

                            foreach (MissionWaypoint wp in missions[0].mission_waypoints)
                            {
                                misS += String.Format("{0}\t{1}\t{2}\t{3}" + Environment.NewLine, wp.order.ToString(), wp.altitude_msl.ToString(), wp.latitude.ToString(), wp.longitude.ToString());
                            }

                            /////
                            String flyS = "";/// Saving Fly Zone to a String
                            flyS = String.Format("AltitudeMinMsl\tAltitudeMaxMsl\tBPtCount" + Environment.NewLine + "BPOrder\tBpLatitude\tBPLongtitude" + Environment.NewLine);

                            foreach (FlyZone fz in missions[0].fly_zones)
                            {
                                flyS += String.Format("{0}\t{1}\t{2}" + Environment.NewLine, fz.altitude_msl_min.ToString(), fz.altitude_msl_max.ToString(), fz.boundary_pts.Count().ToString());
                                foreach (BoundaryPt bp in fz.boundary_pts)
                                {
                                    flyS += String.Format("{0}\t{1}\t{2}" + Environment.NewLine, bp.order.ToString(), bp.latitude.ToString(), bp.longitude.ToString());
                                }
                            }
 
                            /////
                            String sgpS = "";/// Saving Grid Points to a String
                            sgpS = String.Format("GridpointOrder\tAltitudeMsl\tLatitude\tLongitude" + Environment.NewLine);

                            foreach (SearchGridPoint wp in missions[0].search_grid_points)
                            {
                                sgpS += String.Format("{0}\t{1}\t{2}\t{3}" + Environment.NewLine, wp.order.ToString(), wp.altitude_msl.ToString(), wp.latitude.ToString(), wp.longitude.ToString());
                            }


                            /////
                            logging_richtextbox.Text += "Missions Downloaded \n";
                            /////


                            //////////      UPLOADING TELEMETRY DATA        //////////

                            var teldata = new NameValueCollection///Telemetry Data Class
                            {
                                { "latitude", MainV2.comPort.MAV.cs.lat.ToString().Replace(",",".")},
                                { "longitude",  MainV2.comPort.MAV.cs.lng.ToString().Replace(",",".")},
                                { "altitude_msl", MainV2.comPort.MAV.cs.alt.ToString().Replace(",",".") },
                                { "uas_heading", MainV2.comPort.MAV.cs.yaw.ToString().Replace(",",".") }
                            };

                            var response = client.UploadValues("http://" + interop_ip.Text + "/api/telemetry", "POST", teldata);
                            ...
                            double timenow = Math.Round((MainV2.comPort.MAV.cs.datetime.Ticks - timestart) / 10000d);
                            logging_richtextbox.Text = "Interop Refresh Time:" + stopwatch.Elapsed.ToString() + "\n" +
                                "Is Active: " + missions[0].active.ToString() + "\n" +
                                "Moving Obstacles:" + all_obstacles.moving_obstacles.Length + "\n" +
                                "Stationary Obstacles:" + all_obstacles.stationary_obstacles.Length + "\n" +
                                 "Mission Count:" + missions.Count + "\n" +
                                 "Mavlink Packet Count:" + /*MainV2.comPort.MAV.recvpacketcount +*/ "\n" +
                                 "Packet Refresh Time:" + (timenow - lasttime).ToString();
                            lasttime = timenow;

                        }
                        catch
                        {
                            process_richtextbox.Text += "Interoperability can not send or take data\n";
                            interop_process = false;
                            ///interop_start.Text = "Start Interoperability";
                        }
                    }
            ...
            ///process_richtextbox.Text += "Interoperabilty Finished\n";
        }
        ...
        private void refreshWaypoints()
        {
            try
            {
                Overlay.Polygons.Remove(wayPointsZone);
                List<PointLatLng> points3 = new List<PointLatLng>();


                ///Create Polygon and options of itdouble.Parse("3.5", CultureInfo.InvariantCulture)
                wayPointsZone = new GMapPolygon(points3, "Waypoints");
                wayPointsZone.Fill = new SolidBrush(Color.FromArgb(50, Color.Aqua));
                wayPointsZone.Fill = new SolidBrush(Color.Empty);
                wayPointsZone.Stroke = new Pen(Color.Yellow, 1);

                Overlay.Polygons.Add(wayPointsZone);
            }
            catch
            {
                process_richtextbox.Text += "Waypoint Load Error\n";
            }
        }

        private void FlightZoneOverlay_CheckedChanged(object sender, EventArgs e)
        {
            if (FlightZoneOverlay.Checked == true)
            {

                //Add Polygon
                Overlay.Polygons.Add(flightZone);
            }
            else
            {
                Overlay.Polygons.Remove(flightZone);
            }
        }

        private void SearchAreaOverlay_CheckedChanged(object sender, EventArgs e)
        {
            if (SearchAreaOverlay.Checked == true)
            {

                //Add Polygon
                Overlay.Polygons.Add(searchAreaZone);
            }
            else
            {
                Overlay.Polygons.Remove(searchAreaZone);
            }
        }

        private void MissionsOverlay_CheckedChanged(object sender, EventArgs e)
        {
            if (MissionsOverlay.Checked == true)
            {

                Overlay.Markers.Add(AirDropMarker);
                Overlay.Markers.Add(EmergentMarker);
                Overlay.Markers.Add(OffAxisMarker);

            }
            else
            {

                Overlay.Markers.Remove(AirDropMarker);
                Overlay.Markers.Remove(EmergentMarker);
                Overlay.Markers.Remove(OffAxisMarker);
            }
        }

        private void ObtaclesOverlay_CheckedChanged(object sender, EventArgs e)
        {
            if (ObtaclesOverlay.Checked == true)
            {
        ...
        private void waypointOverlay_CheckedChanged(object sender, EventArgs e)
        {
            if (waypointOverlay.Checked == true)
            {
                Overlay.Polygons.Add(wayPointsZone);
            }
            else
